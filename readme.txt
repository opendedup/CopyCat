...
CopyCat is licensed under the Apache License

To user copycat your sdfs volume must meet the following requirements:

Must use an object store backend such as S3,Azure,Google,swift
The SDFS control port for the volume must be visible from the copy cat service
All SDFS volumes in the cluster must use the same object store bucket
All SDFS volumes in the cluster must use the same encryption key
The SDFS volumes must be accessable from the copycat service.
...
mount -t sdfs pool0 /mnt
Step 3 - Get Volume IDs for each volume In this step you will want to verify both volumes are sharing the same pool and get their volume ids. This command can be run on either host.
